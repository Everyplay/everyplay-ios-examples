...
#if TEST_DELAYED_MANUAL_PAUSE_RESUME
static BOOL goingBackground = NO;
#endif
...
#if USE_EVERYPLAY
#if TEST_MANUAL_PAUSE_RESUME || TEST_DELAYED_MANUAL_PAUSE_RESUME
...
- (void)applicationDidBecomeActive:(UIApplication *)application
{
    ...
    if(goingBackground) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 8 * NSEC_PER_SEC), dispatch_get_main_queue(), ^{
            EveryplayLog(@"Doing delayed in-app resume");
            [[[Everyplay sharedInstance] capture] resumeRecording];
        });
    }
    goingBackground = NO;
#endif
#if TEST_MANUAL_PAUSE_RESUME
    EveryplayLog(@"Doing in-app resume");
    [[[Everyplay sharedInstance] capture] resumeRecording];
#endif
#endif
